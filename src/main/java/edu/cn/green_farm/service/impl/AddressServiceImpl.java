...
import edu.cn.green_farm.entity.District;
import edu.cn.green_farm.mapper.AddressMapper;
import edu.cn.green_farm.service.IAddressService;
import edu.cn.green_farm.service.IDistrictService;
...
public class AddressServiceImpl implements IAddressService {
    ...
    public Address getDefaultByUid(Integer uid) {
        return addressMapper.getDefaultByUid(uid);
    }
    ...
    @Override
    public Map getProvinceAndNum(Integer count) {

        // 定义存放省名称和对应数量的集合
        Map<String, Integer> provNameAndNum = new HashMap();
        // 查询省的排名
        List<Address> list = findNumOfProvince(count);
        for (Address address : list) {
            // 获取省的代号
            String province = address.getProvince();
            // 根据省的代号查询省的名称
            String provinceName = getByCode(province);
            // 取到对应的数量
            Integer provinceNum = address.getNum();
            // 放到map中
            provNameAndNum.put(provinceName, provinceNum);
//            provNameAndNum.size();
        }
        // 返回省的名称排名
        return provNameAndNum;
    }
    ...
    private List<Address> findNumOfProvince(Integer count) {
        return addressMapper.findNumOfProvince(count);
    }

    private String getByCode(String province) {
        District district = districtService.getByCode(province);
        String code = district.getName();
        return code;
    }
