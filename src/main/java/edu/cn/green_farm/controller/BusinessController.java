...
import edu.cn.green_farm.controller.exception.FileEmptyException;
import edu.cn.green_farm.controller.exception.FileSizeOutOfLimitException;
import edu.cn.green_farm.controller.exception.FileTypeNotSupportException;
import edu.cn.green_farm.controller.exception.FileUploadException;
...
public class BusinessController extends BaseController {
    @GetMapping("/")
    public String index() {
        return "/web/business/login.html";
    }
    ...
    @PostMapping("/reg.do")
    public ResponseResult<Void> handleReg(Business business) {
    ...
    public ResponseResult<Business> handleLogin(@RequestParam("businessname") String businessname, @RequestParam("password") String password, HttpSession session) {
        // 执行业务端的业务
        Business business = businessService.login(businessname, password);
        // 将用户id和用户名发到session中，保存到服务端
        session.setAttribute("bid", business.getId());
        session.setAttribute("businessname", business.getBusinessname());
        System.err.println(business);
        return new ResponseResult<>(SUCCESS, business);
    }

    @GetMapping("/test")
    public ResponseResult handleTest(HttpSession session) {
        File directory = new File("");
        return new ResponseResult<>(SUCCESS, directory.getAbsoluteFile());
    }
    ...
    public ResponseResult<Void> changePassword(@RequestParam("old_password") String oldPassword, @RequestParam("new_password") String newPassword, HttpSession session) {
        businessService.changePassword(getBidFromSession(session), oldPassword, newPassword);
        return new ResponseResult<>(SUCCESS);
    }

    @RequestMapping("/info.do")
    public ResponseResult<Business> getInfo(HttpSession session) {
        Business business = businessService.getById(getBidFromSession(session));
        return new ResponseResult<>(SUCCESS, business);
    }

    @PostMapping("/change_info.do")
    public ResponseResult<Void> changeInfo(Business business, HttpSession session) {
        business.setId(getBidFromSession(session));
    ...
    public ResponseResult<String> handleUpload(HttpSession session, @RequestParam("file") MultipartFile file) {
        ...
        businessService.changeAvatar(bid, avatar);
        // 返回
